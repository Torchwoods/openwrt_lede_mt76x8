...
int netdev_get_hwinfo(char* ifname)
{
	...
	if (ioctl(fd, SIOCGIFHWADDR, &ifr) < 0) {
		printf("Could not get arptype for '%s'", ifname);
		close(fd);
		return -1;
	}
...
bool netdev_get_mac_address(const char* ifname, unsigned char* mac)
{
	...
	if (ioctl(fd, SIOCGIFHWADDR, &ifr) < 0) {
		printf("MAC addr ioctl failed for '%s'", ifname);
		close(fd);
		return false;
	}
...
bool netdev_get_ip_address(const char* ifname, uint32_t* ip) {
	...
	if (ioctl(fd, SIOCGIFADDR, &ifr) < 0) {
		printf("IP addr ioctl failed for '%s'", ifname);
		close(fd);
		return false;
	}
...
bool netdev_set_ip_address(const char* ifname, uint32_t ip) {
	...
	if (ioctl(fd, SIOCSIFADDR, &ifr) < 0) {
		printf("IP set addr ioctl failed for '%s'", ifname);
		close(fd);
		return false;
	}
...
bool netdev_set_up_promisc(const char *const ifname, bool up, bool promisc)
{
	...
	if (ioctl(fd, SIOCGIFFLAGS, &ifr) < 0) {
		printf("Could not get flags for %s", ifname);
		close(fd);
		return false;
	}
	...
	if (ioctl(fd, SIOCSIFFLAGS, &ifr) < 0) {
		printf("Could not set flags for %s", ifname);
		close(fd);
		return false;
	}
